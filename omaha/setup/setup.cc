...
// Returns the pids of all other BraveUpdate.exe processes with the specified
...
void WriteBraveUpdateUninstallEvent(bool is_machine) {
  CString description;
  SafeCStringFormat(&description, kUninstallEventDescriptionFormat, kAppName);
  BraveUpdateLogEvent uninstall_event(EVENTLOG_INFORMATION_TYPE,
                                       kUninstallEventId,
                                       is_machine);
...
HRESULT Setup::DoProtectedInstall(bool set_keepalive) {
  ...
  if (ShouldInstall()) {
    ...
    hr = DoProtectedBraveUpdateInstall(&setup_files);
    if (FAILED(hr)) {
      SETUP_LOG(LE, (_T("[DoProtectedBraveUpdateInstall fail][0x%08x]"), hr));
...
bool Setup::ShouldOverinstall() {
  ...
  HRESULT hr = goopdate_utils::StartBraveUpdateWithArgs(is_machine_,
                                                         cmd_line,
                                                         address(process));
  if (FAILED(hr)) {
...
HRESULT Setup::DoProtectedBraveUpdateInstall(SetupFiles* setup_files) {
  ASSERT1(setup_files);
  SETUP_LOG(L2, (_T("[Setup::DoProtectedBraveUpdateInstall]")));

  HRESULT hr = StopBraveUpdateAndWait(GetForceKillWaitTimeMs());
  if (FAILED(hr)) {
    SETUP_LOG(LE, (_T("[StopBraveUpdateAndWait failed][0x%08x]"), hr));
...
// TODO(omaha3): Enable. Prefer to move out of Setup if possible.
  ...
  hr = SetupBraveUpdate();
  if (FAILED(hr)) {
    SETUP_LOG(LE, (_T("[SetupBraveUpdate failed][0x%08x]"), hr));
    return hr;
  }
...
void Setup::RollBack(SetupFiles* setup_files) {
  ...
  // TODO(omaha3): Rollback SetupBraveUpdate.
...
HRESULT Setup::SetupBraveUpdate() {
  SETUP_LOG(L2, (_T("[Setup::SetupBraveUpdate]")));

  HighresTimer phase2_metrics_timer;

  omaha::SetupBraveUpdate setup_google_update(is_machine_, is_self_update_);
...
// SetupBraveUpdate, then deletes  the files using SetupFiles.
...
HRESULT Setup::DoProtectedUninstall(bool send_uninstall_ping) {
  const bool can_uninstall = CanUninstallBraveUpdate();
  OPT_LOG(L1, (_T("[CanUninstallBraveUpdate returned %d]"), can_uninstall));

  HRESULT hr = S_OK;
  if (!can_uninstall) {
    hr = GOOPDATE_E_CANT_UNINSTALL;
  } else {
    hr = StopBraveUpdateAndWait(GetForceKillWaitTimeMs());
    if (FAILED(hr)) {
      ...
      SETUP_LOG(L1, (_T("[StopBraveUpdateAndWait returned 0x%08x]"), hr));
      hr = GOOPDATE_E_CANT_UNINSTALL;
    }
  }
  ...
  WriteBraveUpdateUninstallEvent(is_machine_);
...
HRESULT Setup::StopBraveUpdateAndWait(int wait_time_before_kill_ms) {
  HRESULT hr = StopBraveUpdate();
  if (FAILED(hr)) {
    SETUP_LOG(LE, (_T("[StopBraveUpdate failed][0x%08x]"), hr));
    return hr;
  }
...
// method and StopBraveUpdateAndWait after we unify Setup and always have a UI.
HRESULT Setup::WaitForOtherInstancesToExit(const Pids& pids,
  ...
  if (!handles.empty()) {
    ...
    if (WAIT_FAILED == res) {
      ...
      OPT_LOG(LEVEL_ERROR, (_T("[Other BraveUpdate.exe instances failed to ")
...
HRESULT Setup::StartUserCoreProcess(const CString& core_cmd_line) const {
  ...
  if (FAILED(hr)) {
    SETUP_LOG(LE, (_T("[Could not start Brave Update Core][0x%08x]"), hr));
    return hr;
  }
...
bool Setup::CanUninstallBraveUpdate() const {
  CORE_LOG(L2, (_T("[Setup::CanUninstallBraveUpdate]")));
